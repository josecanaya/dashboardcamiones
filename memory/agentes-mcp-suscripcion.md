...
**Verificado:** con API key dummy en el entorno padre, el hijo la ignora (va a suscripción). **Requisito manual del usuario:** `claude auth login` del CLI (el desktop-bundled dio "Not logged in" en headless hasta loguear). Si no está logueado, el chat devuelve error 503 accionable.
