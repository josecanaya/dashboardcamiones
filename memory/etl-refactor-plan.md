...
**Guía ejecutable (2026-07-13):** `docs/migracion/` en la rama `automatizacion` — README (reglas de oro R1-R6, hechos verificados), FASE_0 a FASE_5 (un archivo por fase, pasos con acciones/verificación/commit), PROGRESO.md (checklist). Escrita para que un modelo económico la ejecute sin contexto; los pasos 🛑 STOP-HUMANO marcan decisiones del usuario. Correcciones vs plan original: el golden master YA existía (`etlGoldenMaster.test.ts`, se amplía no se crea) y `RealJourneyDiagnosticsPageLegacy.tsx` SÍ está en uso (App→RealJourneyDiagnosticsPage→RealTruckflowPage) — no borrar. No hay ESLint: el freeze es un script Node `check-arch-rules.mjs` encadenado a npm test.
